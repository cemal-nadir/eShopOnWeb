...
public class OrderBuilder
{
    ...
    public Order WithDefaultValues()
    {
        ...
        _order = new Order(TestBuyerId, new AddressBuilder().WithDefaultValues(), itemList,new OrderState()
        {
    ...
    {
        _order = new Order(TestBuyerId, new AddressBuilder().WithDefaultValues(), new List<OrderItem>(), new OrderState() { State = OrderStateType.Pending, UpdateTime = DateTimeOffset.Now });
        return _order;
    }

    public Order WithItems(List<OrderItem> items)
    {
        _order = new Order(TestBuyerId, new AddressBuilder().WithDefaultValues(), items,new OrderState()
        {
            State = OrderStateType.Pending,
            UpdateTime = DateTimeOffset.Now
        });
